...
| `?` | глобально, вне полей ввода | Открыть справку (`openHelp`) |
...
4. **`Ctrl+S` внутри Task modal**: сохраняет задачу (`tcmSave()`) и закрывает карточку (`tcmClose()`). Работает из любого поля, включая `<textarea>`. Удобно для быстрого сохранения описания или комментария. Блокируем стандартный диалог «Сохранить страницу» через `e.preventDefault()`.
5. **`Ctrl+Enter` в `tcm-chat-text`**: уже есть в `tcmChatKeydown` — не трогаем.
6. **`N` (без модификаторов) глобально**: вызывает `showCreateTask()`. Срабатывает только если фокус **не** в поле ввода (`kbIsTypingTarget` → false) и ни одна модалка не открыта.
7. **`?` (Shift+/) глобально**: вызывает `openHelp()`. Срабатывает только если фокус **не** в поле ввода. Если справка уже открыта — закрывает (`closeHelp`).
8. **IE11**: только `keyCode/which`, никаких `event.key`.
...
function kbOnGlobalKeydown(e) {
    ...
    if (code === 78 && !e.ctrlKey && !e.shiftKey && !e.altKey && !e.metaKey) {
        if (!kbIsTypingTarget(target) && !kbIsHelpOpen() && !kbIsReportOpen() && !kbIsTcmOpen() && !kbIsCreatePanelOpen()) {
    ...
    // ── ? (Shift+/): открыть/закрыть справку ─────────────────────────
    if (code === 191 && e.shiftKey) {
        if (!kbIsTypingTarget(target) && !kbIsReportOpen() && !kbIsTcmOpen() && !kbIsCreatePanelOpen()) {
            if (kbIsHelpOpen()) { closeHelp(); }
            else { openHelp(); }
            try { e.preventDefault(); } catch (er) {}
            return;
        }
...
<table class="kb-help-table">
    ...
    <tr><td><kbd>Enter</kbd></td>                       <td>Сохранить изменения в открытой карточке (не в textarea)</td></tr>
    ...
    <tr><td><kbd>?</kbd></td>                           <td>Открыть / закрыть справку</td></tr>
</table>
```
...
| 3 | Реализовать `Ctrl+S` — сохранить и закрыть карточку | `kanban.js` | В Task modal: `Ctrl+S` из любого поля (включая textarea) сохраняет и закрывает |
| 4 | Реализовать `N` — новая задача и `?` — справка | `kanban.js` | `N` открывает панель создания; `?` открывает/закрывает справку |
| 5 | Help: таблица хоткеев + CSS `<kbd>` | `KanbanBoard_HTML.html`, `kanban.css` | Открыть Help — видна таблица |
| 6 | Документация | `docs/03_*.md`, `docs/05_*.md` | — |
...
- [ ] `?` (Shift+/, вне полей ввода) открывает справку; повторное нажатие — закрывает.
...
1. На доске (фокус не в поле ввода). Нажать `?` (Shift+/).
2. **Ожидаемо:** открылась справка.
3. Нажать `?` ещё раз → справка закрылась.
